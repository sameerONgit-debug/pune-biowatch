  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-3xl border border-[#1a3c2f] bg-gradient-to-br from-[#0f271f] to-[#091a15] shadow-[0_14px_36px_rgba(0,0,0,0.18)] transition-all duration-200 hover:-translate-y-1 hover:border-[#73e5cf]/45 hover:shadow-[0_26px_55px_rgba(0,0,0,0.32)]">
      <div className="flex-1 p-5 sm:p-6">
        ...
        <div className="mt-5 rounded-2xl border border-[#1b4032] bg-[#091a15] p-3.5"><div className="flex items-center justify-between gap-3"><span className="text-[10px] font-extrabold uppercase tracking-[0.12em] text-[#78988a]">Curated climate profile</span><span className="font-mono text-sm font-extrabold text-[#eaf8ed]">{species.severityScore}<span className="text-[10px] text-[#78988a]"> / 100</span></span></div><div className="mt-2 h-2 overflow-hidden rounded-full bg-[#1c3b2f]"><div className={`h-full rounded-full ${scoreColor}`} style={{ width: `${species.severityScore}%` }} /></div></div>
      ...
      <div className="flex items-center justify-between gap-3 border-t border-[#193d2f] bg-[#0b2119] px-5 py-3.5 text-[10px] font-bold text-[#78988a] sm:px-6"><span>{species.regions?.length || 0} documented {species.regions?.length === 1 ? 'habitat' : 'habitats'}</span><span className="hidden font-mono text-[9px] text-[#83a396] sm:inline" title="GBIF occurrence records within the Pune monitoring bounding box">{species.gbifObservationCount != null ? `${species.gbifObservationCount.toLocaleString()} GBIF records` : !species.gbifObservationMode || species.gbifObservationMode === 'loading' ? 'GBIF scan pending' : 'GBIF unavailable'}</span><button onClick={() => onSelect(species)} className="inline-flex items-center gap-1 text-xs font-extrabold text-[#73e5cf] transition-transform group-hover:translate-x-0.5 hover:text-[#d9f99d]">Open dossier <ChevronRight className="h-3.5 w-3.5" /></button></div>
    </article>
  );
}
